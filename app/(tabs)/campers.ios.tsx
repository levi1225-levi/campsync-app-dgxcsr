...
function CampersScreenContent() {
  ...
  const renderCamperItem = useCallback(({ item: camper }: { item: Camper }) => {
    ...
    const statusColor = camper.check_in_status === 'checked-in' || camper.check_in_status === 'checked_in' ? colors.success : camper.check_in_status === 'checked-out' || camper.check_in_status === 'checked_out' ? colors.warning : colors.textSecondary;
